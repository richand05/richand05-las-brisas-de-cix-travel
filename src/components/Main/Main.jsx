...
import { CarFrontFill, ChevronDoubleUp, HouseDoor, ArrowRightSquare } from 'react-bootstrap-icons';
...
const Main = () => {
    ...
    const types = [...new Set(data.places.map(place => place.type))];
    ...
    const sortByDistance = (places) => {
        return places.slice().sort((a, b) => a.distance - b.distance);
    };

    const placesByType = (type) => {
        return data.places.find(place => place.type === type)?.objects || [];
    };

    const filteredPlaces = selectedType === 'all' ?
        sortByDistance(data.places.flatMap(place => place.objects)) :
        sortByDistance(placesByType(selectedType));

    return (
        <div className="main-container pt-5">
            {/* Utiliza el Link de react-router-dom para la navegación */}
            <div style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center" }}><Link to="/nearvy" className="btn-move-page-link mb-4">Cercanos al depa<ArrowRightSquare style={{marginLeft: "10px"}}/> </Link></div>
            {loading ? ( // Mostrar el loader si loading es true
                <div className="loader-container">
                    <div className="spinner-border" role="status">
                        <span className="visually-hidden">Loading...</span>
                    </div>
                    <p>Bienvenidos a "Las Brisas de Cix"</p>
                    <div style={{ color: "#5489EF", background: "white", padding: "4px" }}>Disfruta de tu estadía</div>
                </div>
            ) : (
                <>
                    ...
                    <div className="image-and-cards-container">
                        <div className="cards-container">
                            {filteredPlaces.map((place, index) => (
                                <Card
                                    key={place.id}
                                    index={index + 1} // Sumar 1 al índice para comenzar desde 1 en lugar de 0
                                    title={place.title}
              ...
              <div className="container m-3 d-flex flex-column justify-content-center align-items-center p-3" style={{ background: "#083758", color: "white", textAlign: "center" }}>
                        <p>Estos sitios son recomendaciones</p>
                        <p>El tiempo que se muestra es aproximado, teniendo como punto de partida el departamento</p>
                    </div>
        </div>
    );
};
